...
function nvimex 
    python2 $__neovim_plugin_base/bin/nvimex.py $argv
end

# Prompt addin for nvimex mode
function __nvim_prompt_addin
    printf (set_color blue)'nvim'
end
...
function __neovim_define_helpers
    ...
    function __neovim_pass_to_vim -e fish_command_not_found
        if set __command (echo $argv | string match -r '^:(.*)')[2]
            nvimex $__command
        end
    end

    fish_prompt_breadcrumb add __nvim_prompt_addin
end
...
function __neovim_erase_helpers
    ...
    fish_prompt_breadcrumb rm __nvim_prompt_addin
end
